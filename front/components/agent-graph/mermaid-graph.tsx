    ...
    return (
      <div className="flex h-64 items-center justify-center text-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Agent workflow will appear here during analysis</p>
        </div>
      </div>
    )
  }
...
function generateMermaidDefinition(nodes: AgentNode[], activeNodeId: string | null): string {
  const lines: string[] = ["flowchart TD"]

  // Add styling classes
  lines.push("  classDef active fill:#6366f1,stroke:#4f46e5,stroke-width:2px,color:#fff")
  lines.push("  classDef complete fill:#22c55e,stroke:#16a34a,stroke-width:2px,color:#fff")
  lines.push("  classDef pending fill:#334155,stroke:#475569,stroke-width:1px,color:#94a3b8")
  lines.push("")

  // Add nodes with properly escaped labels
  nodes.forEach((node) => {
    // Use simple text without HTML - Mermaid doesn't support HTML in all node types
    const label = `${node.name} - ${node.tokens}tok ${node.duration}ms`

    // Use different shapes based on node type
    // Diamond for orchestrator, rounded rect for others
