...
function generateMermaidDefinition(
  ...
  nodes.forEach((node) => {
    ...
    const htmlLabel =
      `<div style="padding: 8px; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #f1f5f9;"><span style="font-weight: 600; font-size: 1.1em; margin-top: 4px;">${node.name}</span><span style="font-size: 0.8em; opacity: 0.8; margin-top: 2px;">${
        (node.duration / 1000).toFixed(1)
      }s &bull; ${node.tokens}tok</span></div>`;
