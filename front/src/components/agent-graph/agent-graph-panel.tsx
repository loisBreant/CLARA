...
import { MermaidGraph } from "./mermaid-graph.tsx";
import { AgentNodeList } from "./agent-node-list.tsx";
import { MetricsPanel } from "./metrics-panel.tsx";
import { BarChart3, GitBranch, List } from "lucide-react";
...
export function AgentGraphPanel(
  { nodes, activeNodeId, metrics }: AgentGraphPanelProps,
) {
  ...
  return (
    <div className="flex flex-1 min-w-0 min-h-0 flex-col bg-card/50">
        ...
        <div className="flex items-center gap-4 text-xs">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-muted-foreground">
              {completedNodes} complete
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="h-2 w-2 rounded-full bg-primary animate-pulse" />
            <span className="text-muted-foreground">
              {nodes.length - completedNodes} running
            </span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <TabsContent
          value="graph"
          className="relative flex-1 m-0 p-0 flex-col min-h-0 data-[state=active]:flex"
        >
        ...
        >
          <div className="absolute inset-0 overflow-y-auto p-6">
            <MetricsPanel nodes={nodes} metrics={metrics} />
          </div>
        </TabsContent>
      </Tabs>
