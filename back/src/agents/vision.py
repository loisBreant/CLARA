...
"""
        super().__init__(system_prompt, AgentType.EXECUTOR, model="google/gemma-3-27b-it")

    def analyze(self, instruction: str, image_path: str, metrics: AgentsMetrics) -> Generator[AgentResponse, None, None]:
        self.reset_id()
        ...
        if image_path.startswith("/static"):
             ...
             project_back_dir = os.path.abspath(os.path.join(base_dir, "../../")) # back/
             real_path = os.path.join(project_back_dir, image_path.lstrip("/"))
        
        if not os.path.exists(real_path):
             yield AgentResponse(metrics=metrics, id=self.agent_data.id, chunk=f"Error: Image file not found at {real_path}")
             return
        ...
        try:
            ...
            yield AgentResponse(metrics=metrics, id=self.agent_data.id, chunk=f"Error reading image: {e}")
            return
        ...
        try:
            ...
            yield AgentResponse(metrics=metrics, id=self.agent_data.id, chunk=f"Error during vision analysis: {e}")
